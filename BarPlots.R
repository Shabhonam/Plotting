...
Stack_data <- read.csv("Klebs_data.csv")
...
ggplot(df_long[order(df_long$value, decreasing = F),],  aes(x = ID,  y = value,  fill =Species )) +
geom_bar(stat ="identity" ,  position = position_fill())+ theme(legend.position="bottom" ,  legend.direction="horizontal" ,  legend.title = element_blank())+
labs(x="" , y="Read counts %")+theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) + scale_fill_manual(values = familyPalette)
dev.off()
